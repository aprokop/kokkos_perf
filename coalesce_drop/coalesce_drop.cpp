...
{
public:
  typedef Kokkos::StaticCrsGraph<local_ordinal_type, Kokkos::LayoutLeft,
                                 device_type>
      local_graph_type;
...
public:
  ...
  bool operator()(local_ordinal_type row, local_ordinal_type col,
                  scalar_type val) const
  {
    // We avoid square root by using squared values
    auto aiiajj = ATS::magnitude(diag(row, 0)) *
                  ATS::magnitude(diag(col, 0));            // |a_ii|*|a_jj|
...
public:
  DistanceLaplacianDropFunctor(GhostedViewType lapl_diag,
...
private:
  GhostedViewType
      diag; // corresponds to overlapped diagonal multivector (2D View)
  DistanceFunctor distFunctor;
  magnitudeType eps;
};

template <class scalar_type, class local_ordinal_type, class MatrixType,
...
public:
  ScalarFunctor(MatrixType A_, DropFunctorType dropFunctor_,
                ...
                typename vals_type::non_const_type valsAux_, bool reuseGraph_,
                bool lumping_, scalar_type threshold_, const int rows_per_team_)
      : A(A_)
  ...
  void operator()(const team_member &dev, local_ordinal_type &nnz) const
  {
    Kokkos::parallel_for(
        ...
        [&](const local_ordinal_type &loop) {
          const local_ordinal_type row =
              static_cast<local_ordinal_type>(dev.league_rank()) *
                  rows_per_team +
              loop;
          if (row >= A.numRows())
            return;
          ...
          Kokkos::parallel_reduce(
              ...
              [&](const local_ordinal_type &colID,
                  reduction_type &local_reduce_pair) {
...
int64_t cd_launch_parameters(int64_t numRows, int64_t nnz,
...
void kernel_coalesce_drop_device(
    int loop_cnt,
    KokkosSparse::CrsMatrix<scalar_type, local_ordinal_type, device_type> A,
    const std::string &algo, scalar_type threshold, bool lumping,
    int rows_per_team, int vector_length)
{
  using local_matrix_type =
      KokkosSparse::CrsMatrix<scalar_type, local_ordinal_type, device_type>;
  using kokkos_graph_type =
  ...
  cols_type colsAux(Kokkos::ViewAllocateWithoutInitializing("FA_aux_cols"),
                    nnzA);
  vals_type valsAux(Kokkos::ViewAllocateWithoutInitializing("FA_aux_vals"),
                    nnzA);
  ...
  if (vector_length < 0 || rows_per_team < 0)
  {
    ...
    rows_per_team = cd_launch_parameters<execution_space>(
  ...
  if (team_size < 0)
    team_policy =
        Kokkos::TeamPolicy<execution_space, Kokkos::Schedule<Kokkos::Static>>(
            numRows, Kokkos::AUTO, vector_length);
  else
    team_policy =
        Kokkos::TeamPolicy<execution_space, Kokkos::Schedule<Kokkos::Static>>(
            numRows, team_size, vector_length);

  if (algo == "classical")
  {
    // Construct overlapped matrix diagonal
    Kokkos::View<double **, device_type> ghostedDiagView(
        "ghosted_diag", numRows, 1); // MultiVector
    // FIXME: extract matrix diagonal

    ClassicalDropFunctor<local_ordinal_type, decltype(ghostedDiagView)>
        dropFunctor(ghostedDiagView, threshold);
    ScalarFunctor<scalar_type, local_ordinal_type, local_matrix_type,
                  decltype(dropFunctor)>
        scalarFunctor(A, dropFunctor, rows, colsAux, valsAux, reuseGraph,
                      lumping, threshold, rows_per_team);

    // Rule of thumb for GPU: vector length (3rd parameter to team policy) to be
    // 1/3 of row length, and has to be a power of 2
  ...
  else if (algo == "distance laplacian")
  {
    ...
    Kokkos::View<scalar_type **, device_type> ghostedCoordsView(
        "ghosted_coords", numRows, 3); // MultiVector

    DistanceFunctor<local_ordinal_type, decltype(ghostedCoordsView)>
        distFunctor(ghostedCoordsView);

    Kokkos::View<scalar_type *, device_type> lapl_diag("lapl_diag", numRows);

    // Construct Laplacian diagonal
    Kokkos::parallel_for(
        "construct_lapl_diag", range_policy(0, numRows),
    ...
    // Filter out entries
    DistanceLaplacianDropFunctor<local_ordinal_type, decltype(lapl_diag),
                                 decltype(distFunctor)>
  ...
  // std::cout << "Number of dropped entries = " << (1.0 - (double)nnzFA/nnzA)
  // << std::endl;
...
kernel_construct(local_ordinal_type numRows)
{
  ...
  typedef KokkosSparse::CrsMatrix<scalar_type, local_ordinal_type, device_type>
      local_matrix_type;

  return local_matrix_type("A", numRows, numCols, nnz, values.data(), rowPtr,
                           colInd.data(), false /*pad*/);
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (!strcmp(argv[i], "--lumping"))
    {
      lumping = true;
    }
    else if (!strcmp(argv[i], "--no-lumping"))
    {
      lumping = false;
    }
    else if (!strcmp(argv[i], "-r") || !strcmp(argv[i], "--rows_per_team"))
    {
      rows_per_team = atoi(argv[++i]);
    }
    else if (!strcmp(argv[i], "-v") || !strcmp(argv[i], "--vector_length"))
    {
    ...
    else if (!strcmp(argv[i], "-e") || !strcmp(argv[i], "--eps"))
    {
      eps = atof(argv[++i]);
    }
  ...
  if (!study)
  {
    ...
    for (int i = 0; i < num_loops; i++)
      kernel_coalesce_drop_device(i, A, algo, eps, lumping, rows_per_team,
                                  vector_length);
